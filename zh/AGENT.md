...
`AGENTS.md` 是有意保持很薄的兼容入口，用来把 Codex 风格的 agent 指向本指南。
...
入口文件给 Agent 提供继续工作的最小上下文。`SKILL.md` 应保持可执行且简洁。`README.md` 面向人类解释 CodeBTI。`AGENT.md` 定义仓库维护预期。`AGENTS.md` 是兼容入口，应保持简短。

### 配套 Skills

`neat-freak/` 是独立的 assistant skill package，用于会话结束后的文档和记忆同步。它的 CodeBTI overlay 会检查 manifest 漂移、中文翻译镜像、examples 和 fixtures、验证门禁，以及已安装 skill 副本。保留 `neat-freak/references/` 中的 upstream-style references，并在 `neat-freak/zh/` 中维护本地中文入口和 reference mirror。
