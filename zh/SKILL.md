...
9. 从 `python/profiles/python-profile-taxonomy.md` 和 `python/patterns/` 中的模式数据库推断 Python Profile。
10. 选择支撑最终建议的本地模式/资源引用。优先选择 `python/patterns/python/` 下的具体文件，仅包含对指导有影响的引用。
...
- 在最终报告中，用相关本地模式页面的引用来支撑，例如 `python/patterns/python/facade.md`，当框架或库选择影响了指导时，加上有用的资源链接。
